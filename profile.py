...
async def update_profile(db: db_dependency, req: UsernameRequest, current_user: dict = Depends(get_current_user)):
    ...
    db.commit()
    db.refresh(user)
    return {"message": "Profile updated successfully", "username": user.username, "name": user.name}
...
async def find_by_phone(phone: str, db: db_dependency, current_user: dict = Depends(get_current_user)):
    user = db.query(User).filter(User.number == phone).first()
...
async def search_users(q: str, db: db_dependency, current_user: dict = Depends(get_current_user)):
    if not q.strip():
        return []
    users = db.query(User).filter(User.username.ilike(f"%{q}%"),
                                  User.id != current_user['id']).limit(20).all()
    return [{"id": u.id, "username": u.username, "number": u.number, "name": u.name}
